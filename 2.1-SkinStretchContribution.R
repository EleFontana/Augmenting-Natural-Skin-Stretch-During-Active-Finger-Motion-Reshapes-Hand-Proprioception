...
# ---------------------------
# MEAN PLATEAU ANGLE FOR ALL CYLINDERS
# DevOff vs DevOn
# ---------------------------
angle_summary <- all_data %>%
  filter(
    Condition %in% c("DevOff", "DevOn")
  ) %>%
  group_by(Cylinder, Condition) %>%
  summarise(
    mean_angle = mean(PlateauAngle, na.rm = TRUE),
    sd_angle   = sd(PlateauAngle, na.rm = TRUE),
    n          = sum(!is.na(PlateauAngle)),
    .groups = "drop"
  )

# Extract mean angles
DevOffAngle <- angle_summary %>%
  filter(Condition == "DevOff") %>%
  pull(mean_angle)

DevOnAngle <- angle_summary %>%
  filter(Condition == "DevOn") %>%
  pull(mean_angle)

# Difference in joint angle (Δγ)
deltaGamma <- DevOffAngle - DevOnAngle
...
# ---------------------------
# STRETCH ESTIMATION (linear regression, see Methods)
# ---------------------------
beta   <- 0.18
offset <- 9
gain_DevOn <- 1.5

DevOffStretch <- beta * (DevOffAngle - offset)
DevOnStretch  <- gain_DevOn * DevOffStretch

# Difference in stretch (ΔA₀)
deltaAzero <- DevOffStretch - DevOnStretch

# ---------------------------
# DERIVATIVE OF THE STRETCH–ANGLE RELATION
# stretch(θ) = β (θ − offset), for θ > offset
# ---------------------------
l <- beta
...
# COMPUTE WEIGHT [%]
# ---------------------------
wC <- - (l * deltaGamma) * 100 / deltaAzero
print(wC)
