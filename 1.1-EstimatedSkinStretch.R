...
# ---------------------------
# PARAMETERS FOR STRETCH ESTIMATION (see Methods)
# ---------------------------
offset <- 9
beta <- 0.18
gain_DevOn <- 1.5   # additional gain applied in the DevOn condition

# ---------------------------
# COMPUTE ESTIMATED SKIN STRETCH
# Condition-specific conversion is applied here
# ---------------------------
stretch_data <- all_data %>%
  filter(Condition %in% c("DevOff", "BareHand", "DevOn")) %>%
  mutate(
    stretch = case_when(
      Condition == "DevOn" ~ gain_DevOn * beta * (PlateauAngle - offset),
      TRUE                 ~ beta * (PlateauAngle - offset)
    )
  )

# ---------------------------
# COMPUTE MEAN STRETCH PER CONDITION
# ---------------------------
mean_stretch <- stretch_data %>%
  group_by(Condition) %>%
  summarise(
    mean_stretch = mean(stretch, na.rm = TRUE),
    sd_stretch = sd(stretch, na.rm = TRUE),
    .groups = "drop"
  )


test_stretch <- stretch_data %>%
  group_by(Cylinder, Condition) %>%
  summarise(
    mean_stretch = mean(stretch, na.rm = TRUE),
    sd_stretch = sd(stretch, na.rm = TRUE),
    .groups = "drop"
...
# ---------------------------
# COMPUTE REFERENCE STRETCH (Cylinder 1)
# ---------------------------
reference_stretch <- stretch_data %>%
  filter(Cylinder == 1) %>%
  group_by(Condition) %>%
  summarise(
    ref_stretch = mean(stretch, na.rm = TRUE),
    .groups = "drop"
  )

# ---------------------------
# COMPUTE PERCENTAGE RATIO RELATIVE TO CYLINDER 1
# ---------------------------
stretch_summary <- mean_stretch %>%
  left_join(reference_stretch, by = "Condition") %>%
  mutate(
    percent_of_reference = (mean_stretch / ref_stretch) * 100
  )

print(stretch_summary)