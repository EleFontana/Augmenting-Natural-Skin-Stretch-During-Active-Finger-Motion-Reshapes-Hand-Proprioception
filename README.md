...
Participants performed the hand-matching task under three conditions. 
In DevOff condition, the device remained inactive. In BareHand condition, participants performed the task without wearing the device. In DevOn condition, the device delivered movement-contingent skin stretch. PIP joint angles were recorded either via the encoder embedded in TWIST (DevOff and BareHand conditions) or using a Leap Motion sensor mounted on opaque goggles (condition B), which prevented visual feedback of the environment while capturing accurate joint kinematics. 
